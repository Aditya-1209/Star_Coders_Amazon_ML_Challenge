# r4: run the improved v2 / two-hop pipeline
...
- Two missing addresses contribute zero graph similarity. This changes stage-3
  features, so **retrain stage 3**. Models without `feature_version: r4` in their
  stage-3 metadata are rejected at inference.
...
These are correctness and resource-use changes. A higher F0.5 score must be
established by a new run, not assumed from the code changes.
...
Use **`work/r4`** so the previous run and shipped weights stay intact. Keep this
...
```text
python -m er_v2.translit --out work/r4/models/translit.json
python -m er_v2.prepare --work work/r4 --workers 12 --translit work/r4/models/translit.json
python -m er_v2.run_block --work work/r4 --split train --top-k 64
python -m er_v2.run_block --work work/r4 --split test --top-k 64
python -m er_v2.run_features --work work/r4 --split train --shard-pairs 2000000 --workers 12
python -m er_v2.run_features --work work/r4 --split test --shard-pairs 2000000 --workers 12
python -m er_v2.train --work work/r4 --model-dir work/r4/models --device cuda --threads 12 --batch-rows 250000
python -m er_v2.stage3 --work work/r4 --model-dir work/r4/models --split train --device cuda --threads 12 --batch-rows 250000 --support-anchors 5000
```
...
Stage 1 fits folds 0/1; stage 2 fits folds 2/5; stage 3 fits folds 6/7. Fold 3 is
used for tuning and fold 4 for reporting. The learned transliteration map
...
Inspect `work/r4/models/metrics.json` and `stage3_metrics.json`. The latter
reports stage-3, stage-2 reference, and candidate-oracle scores using the selected
model directory. The historical 0.9494 score is not an r4 result.

## 4. Predict and validate

Stage 2 creates `work/r4/test_preds.parquet`, which stage 3 needs:

```text
python -m er_v2.predict --work work/r4 --model-dir work/r4/models --output output/r4-stage2 --device cuda --threads 12 --batch-rows 250000
python -m er_v2.stage3 --work work/r4 --model-dir work/r4/models --split test --output output/r4 --device cuda --threads 12 --batch-rows 250000 --support-anchors 5000
python student_resource/utils/validate_submission.py --matching output/r4/matching_results.tsv --candidate output/r4/candidate_pairs.tsv --test-dir student_resource/dataset/test --check-ids
```

The final stage-3 files are under **`output/r4/`**. The candidate file is exactly
...
workspace if feature generation exhausts RAM. `--threads` controls XGBoost and
...
```text
python -m er_v2.package --output output/r4 --model-dir work/r4/models --zip submissions/Star_Coders_r4_submission.zip
```
