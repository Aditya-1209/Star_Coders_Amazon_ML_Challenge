...
def macro_f05(pred: pl.DataFrame, truth: pl.DataFrame, anchors: pl.Series) -> dict:
    ...
    a = pl.DataFrame({"sidx": anchors.cast(pl.UInt32)})
    pred = pred.select(pl.col("sidx").cast(pl.UInt32), pl.col("tidx").cast(pl.UInt32)).join(a, on="sidx")
    truth = truth.select(pl.col("sidx").cast(pl.UInt32), pl.col("tidx").cast(pl.UInt32)).join(a, on="sidx")
