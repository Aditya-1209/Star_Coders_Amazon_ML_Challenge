...
def normalize_frame(df: pl.DataFrame, pool: Pool, chunk: int = 25_000,
                    buffer_rows: int = 250_000) -> pl.DataFrame:
    # Bound Python strings and queued multiprocessing payloads to one window,
    # rather than converting and queuing every address in a 5-million-row file.
    output = []
    for window in df.iter_slices(buffer_rows):
        ...
        for f, c, a in pool.imap(_norm_chunk, jobs):
            ...
            core.extend(c)
            addr.extend(a)
        output.append(window.with_columns(pl.Series("name_n", full),
                                           pl.Series("core_n", core), pl.Series("addr_n", addr)))
    if not output:
        return df.with_columns(*[pl.Series(name, [], dtype=pl.String) for name in ("name_n", "core_n", "addr_n")])
...
def main() -> None:
    ...
    ap.add_argument("--workers", type=positive_int, default=min(os.cpu_count() or 1, 8))
    ap.add_argument("--buffer-rows", type=positive_int, default=250_000)
