...
class FrameIter(xgb.DataIter):
    ...
    def next(self, input_data) -> bool:
        ...
        weights = {"weight": batch["w"].to_numpy()} if "w" in batch.columns else {}
        input_data(data=self._batch_features, label=self._batch_labels, feature_names=self.features, **weights)
        return True
